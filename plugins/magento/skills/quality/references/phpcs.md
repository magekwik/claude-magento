...
```bash
vendor/bin/phpcs --standard=Magento2 --report=csv <paths>
```

Human-readable variant: `vendor/bin/phpcs --standard=Magento2 --report=full <paths>` (add `-s` to print the sniff code under each message, `-p` for progress, `--basepath=.` to shorten paths, `--colors`). `<paths>` are files or directories — `app/code/Acme/Catalog`, a list of changed files, or nothing at all when a `phpcs.xml.dist` names `<file>` entries.

Useful options:

| Option | Effect |
|---|---|
| `--severity=10` | show only severity-10 messages (all of them are errors in this standard); `--error-severity=N` / `--warning-severity=N` set the thresholds separately (default 5); `-n` = `--warning-severity=0` |
...
| `-q` | quiet: no progress, no deprecation banner. PHP_CodeSniffer ≥ 3.9 prints `DEPRECATED: Scanning CSS/JS files is deprecated …` lines for every `Magento2.Less.*` sniff *before* any report, on stdout — pass `-q` whenever a script parses the output |
...
Consequences: `--severity=10` shows exactly the error set; the default threshold (5) shows everything; there is no severity below 5 except a handful of codes disabled with `<severity>0</severity>`.
...
- run: |
    ...
    [ -z "$files" ] || vendor/bin/phpcs -q --standard=Magento2 --report=checkstyle --report-file=phpcs.xml --report=summary $files
```

Whole-module runs belong in a nightly job, not on every push; `--cache` and `--parallel` make even those tolerable. `--report=checkstyle` is what annotation actions and IDEs consume.
...
- Verified in a 2.4.9 install: `vendor/magento/magento-coding-standard/composer.json` and `Magento2/ruleset.xml` (severity bands, types, extensions arg, `testVersion`), `Magento2/Sniffs/*` (family listing), `vendor/bin/phpcs --standard=Magento2 -e` (308 sniffs), `vendor/bin/phpcs --config-show` (installer-written `installed_paths`), `vendor/squizlabs/php_codesniffer/src/Runner.php` (exit codes), `magento/magento2` root `composer.json` at tags 2.4.4–2.4.9 (`require-dev`)
