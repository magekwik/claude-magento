...
- **A6.** Schema is declarative only: `etc/db_schema.xml` + `db_schema_whitelist.json`; data changes are `DataPatchInterface` classes. No `InstallSchema`/`UpgradeSchema`/`InstallData` in new code — *legacy scripts are deprecated since 2.3 and do not diff.* PHPCS: Magento2.Legacy.InstallUpgrade
- **A7.** Never edit `vendor/` or `app/code/Magento`. Patch third-party code only via `cweagans/composer-patches` with the `.patch` file committed — *edits vanish on the next `composer install`.* PHPCS: —
...
- **A9.** Constructor injection only; use generated `Factory` classes for non-injectable objects (models, DTOs) and `Proxy` for heavy dependencies of frequently instantiated classes, wired in `di.xml` rather than type-hinted — *`new` bypasses DI; unproxied heavy deps slow every request.* PHPCS: Magento2.Classes.DiscouragedDependencies
...
- **S2.** Frontend controllers that mutate state implement `HttpPostActionInterface` (and `CsrfAwareActionInterface` only when a custom check is needed); GET never mutates — *the form key check is keyed to these interfaces.* PHPCS: —
...
- **P3.** Keep pages full-page-cacheable: no `cacheable="false"` on layout blocks unless the content is truly per-user; per-user content goes through customer-data sections (Luma) or private-content events (Hyvä) — *one uncacheable block disables FPC for the page.* PHPCS: —
- **P4.** No I/O or heavy work in constructors; plugins on hot paths (`ProductRepository::get`, `Layout::renderElement`) must be cheap — *constructors run on every DI resolution.* PHPCS: —
...
- **L3.** JavaScript is RequireJS modules registered in `requirejs-config.js`, initialised with `x-magento-init`/`data-mage-init`; extend core widgets with mixins; no inline globals — *load order and the jQuery lifecycle.* PHPCS: —
...
- **H2.** Before writing any frontend for a third-party module, check for a Hyvä compatibility module (`hyva-themes/magento2-*` or a `*-hyva` module) and follow its patterns — *Luma templates from extensions do not render in Hyvä.* PHPCS: —
...
- **H4.** Data comes from ViewModels; per-customer data uses private content (`private-content-loaded` event) not sections.xml/Knockout — *FPC-safe.* PHPCS: —
...
- **Q5.** User-facing strings go through `__()` with entries in `i18n/en_US.csv`; never concatenate translated fragments — *translation and pluralisation.* PHPCS: Magento2.Translation.ConstantUsage
