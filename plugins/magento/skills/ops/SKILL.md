...
| PHP class body | nothing — unless a constructor signature changed and `generated/metadata/` exists from an earlier compile: `rm -rf generated/code generated/metadata` | `setup:di:compile` (regenerates interceptors, factories, proxies and the compiled DI) |
...
When answering "what do I run" for a developer-mode project, say it in these words: **`setup:di:compile` is not needed in developer mode — it is required only in production mode**, and **`setup:static-content:deploy` is not needed in developer mode** either (static files are materialised on request). Do not list either as a step; mention them only to say they are not needed.
...
1. `cache:clean [types]` removes only entries tagged by the listed Magento cache types; `cache:flush [types]` wipes the whole storage behind them — on Redis that is `FLUSHDB`, which also erases anything another application keeps in that database. Default to `clean`; reach for `flush` when `clean` did not help or the storage is dedicated.
...
4. `deploy:mode:set production` wipes `var/cache`, `generated/code`, `generated/metadata`, `var/view_preprocessed` and `pub/static`, then compiles and deploys static content for the admin users' locales; `-s` only writes `MAGE_MODE`. Run it deliberately, in maintenance mode. `deploy:mode:set developer` leaves `generated/` alone — delete it yourself when coming from production.
...
12. Version upgrades (any 2.4.x target from a Composer 2 install, i.e. 2.4.2 or later; from 2.4.0/2.4.1 go to 2.4.2 with Composer 1 first): `composer require magento/composer-root-update-plugin ~2.0 --no-update && composer update`, then `composer require-commerce magento/product-community-edition 2.4.9 --no-update && composer update` (same syntax for `2.4.9-p1` security releases), then the production sequence below. Plain `composer require magento/product-community-edition=2.4.9 --no-update` still resolves, but does not reconcile the root `composer.json` — `require-commerce` does.
...
Why this shape: `composer install` (never `update`) reproduces the tested `composer.lock`; compiling and deploying static content before `setup:upgrade` means the DB is the last thing to move and everything on disk already matches the new code, which is what `--keep-generated` assumes; static deploy names the storefront theme and `Magento/backend` explicitly so nothing is built for themes nobody serves; `cache:flush` at the end throws away pages and config cached under the old release. Adobe's single-machine page orders it `setup:upgrade` → `setup:di:compile` → `setup:static-content:deploy` → `cache:clean` without `--keep-generated` — also correct; the one wrong combination is `--keep-generated` with a `generated/` that predates the code. Cron and queue consumers stop for the duration (`cron:remove` or comment the crontab line; `cron:run --group=consumers` to drain queues first).
