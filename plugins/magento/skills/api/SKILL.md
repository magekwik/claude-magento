...
7. Exceptions → status: `NoSuchEntityException` 404; `AuthorizationException`/`AuthenticationException` 401 (a missing ACL resource is 401, not 403); any other `LocalizedException` (`InputException`, `CouldNotSaveException`, `StateException`) 400 with its message; `Magento\Framework\Webapi\Exception` its own code (backpressure 429). Anything else is masked to `Internal Error. Details are available in Magento log file. Report ID: …` — 500, except `\InvalidArgumentException`, `\UnexpectedValueException`, `\BadMethodCallException` and DB exceptions, which are 400 but masked too. Developer mode adds `trace`.
8. **A8** — `webapi_rest`, `webapi_soap` and `graphql` are DI areas (`etc/webapi_rest/di.xml`, `etc/graphql/di.xml`); a plugin on the service interface in global `di.xml` fires for REST, GraphQL and PHP callers alike because they all call the same object.
