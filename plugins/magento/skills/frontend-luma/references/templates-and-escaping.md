...
- A raw `<script>` with inline code is what CSP is there to stop. The storefront runs CSP in report-only mode by default, but since 2.4.7 the payment pages (`checkout_index_index`, multishipping billing, admin order create) run in *restrict* mode with inline scripts disallowed, so inline code there is blocked unless rendered through `$secureRenderer->renderTag('script', [], $js, false)` (adds the nonce) or `$secureRenderer->renderEventListenerAsTag('onclick', $js, '.selector')`. `<script type="text/javascript">` is flagged by `Magento2.Legacy.PhtmlTemplate`; use `<script>` (via the renderer) or the `x-magento-init` type.
