...
| Scripts | `build` (= `NODE_ENV=production npx tailwindcss --postcss -i tailwind-source.css -o ../css/styles.css --minify`), `build-prod` (same + "Done!"), `watch`, `browser-sync` | `build` (= `npm run generate && npx tailwindcss -i tailwind-source.css -o ../css/styles.css --minify` from 1.4.7; 1.4.0–1.4.6 run `generate` through `prebuild`/`prewatch` hooks instead), `generate` (= `npx hyva-sources && npx hyva-tokens`), `watch`, `start`, `browser-sync`; `build-prod` alias again from 1.4.7 |
...
| `hyva.config.json` | — | `tailwind.include`/`exclude` (`[{ "src": "vendor/hyva-themes/magento2-default-theme" }]`) and `tokens.values` (`color.primary`, …) read by `npx hyva-sources` / `npx hyva-tokens`. As shipped, `include` is empty (1.4.0) or holds the parent only as an `"example_src"` placeholder (1.5.x): a copied `web/` scans no parent templates until you rename it to `"src"` |
...
Alpine 3 is loaded as `Hyva_Theme::js/alpine3.min.js` (a `defer` script) from the `script-alpine-js` block in `before.body.end`, so components initialise after the DOM is parsed. The directives Hyvä templates use:
...
- https://docs.hyva.io/hyva-themes/writing-code/csp/index.html and https://docs.hyva.io/hyva-themes/writing-code/csp/csp-compatibility.html — Alpine CSP build, `$hyvaCsp->registerInlineScript()` placement (the page assumes theme module ≥ 1.3.11), `$hyvaCsp` availability per theme
...
- https://github.com/hyva-themes/magento2-default-theme-csp — `composer.json` at tag 1.3.10 (2024-12-06) requires `hyva-themes/magento2-theme-module >=1.3.10`, so the CSP theme does not guarantee `$hyvaCsp`
- https://github.com/hyva-themes/magento2-theme-module — `CHANGELOG.md` (1.3.11 "Support strict CSP mode" adds `HyvaCsp`/`$hyvaCsp`; absent from the 1.3.10 tree), `src/view/frontend/templates/page/js/hyva.phtml` (helper definitions), `private-content.phtml` (storage keys, cookies, `customer/section/load`, event dispatch), `variables.phtml` (`BASE_URL`, `CURRENT_STORE_CODE`), `layout/default_hyva.xml` (`head.additional` / `before.body.end` blocks), `ViewModel/*` (registry, icons), `view/base/web/js/alpine3.min.js` (Alpine 3.14 in 1.5.2; `.away` still handled); tags 1.3.22 and 1.5.2
