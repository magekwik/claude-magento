...
- FRONTEND = `Hyvä` if FACTS.hyva is true or any theme parent starts with `Hyva/` (`Hyva/default`, `Hyva/default-csp`, …); otherwise `Luma`.
- LUMA = `true` if FACTS.hyva is false, or any frontend theme's parent does not start with `Hyva/`, or there are no themes; else `false`.
- HYVA = FACTS.hyva as `true`/`false`.
- PREFIX by FACTS.env: `warden` → `warden env exec php-fpm`; `ddev` → `ddev exec`; `docker` → `docker compose exec <php-service>` (append the sentence "— replace `<php-service>` with the PHP service name from your compose file"); `docker-magento` → no prefix: the project's own `bin/magento` wrapper runs inside the container, so the Environment line reads ``run CLI as `bin/magento …` (docker-magento wrapper)``; `native` → no prefix: the Environment line reads ``run CLI as `bin/magento …` `` with nothing before `bin/magento`.
...
```
<!-- magento:begin -->
## Magento project (generated by the magento plugin vVERSION — rerun /magekwik-magento:init to refresh)

- **Edition:** EDITION FACTS.magento.version · host PHP FACTS.php · mode: FACTS.mode
...
- **Environment:** <Warden | DDEV | Docker Compose | docker-magento | native> — run CLI as `PREFIX bin/magento …` (docker-magento and native: PREFIX is empty — write `bin/magento …`, with the backtick immediately followed by `bin/magento`, no leading space; docker-magento appends ` (docker-magento wrapper)`)
